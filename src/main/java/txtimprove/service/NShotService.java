...
@Service
public class NShotService {

    private final ChatClient chatClient;
    private final MessageService messageService;

    public NShotService(ChatClient.Builder chatClientBuilder, MessageService messageService) {
    ...
    public String getDefaultPrompt() {
        return messageService.getMessage("nshot.prompt.default");
    }

    public String generateSolution(List<String> problems, List<String> solutions, String newProblem, String customPrompt, String selectedModel) {
        try {
            ...
            for (int i = 0; i < problems.size() && i < solutions.size(); i++) {
                examplesBuilder.append("Problem: ").append(problems.get(i))
                              .append("\nSolution: ").append(solutions.get(i))
                              .append("\n\n");
            }
            
            // Replace placeholders in prompt
            String finalPrompt = prompt
                .replace("{examples}", examplesBuilder.toString().trim())
                .replace("{new_problem}", newProblem);
